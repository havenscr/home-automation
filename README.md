...
- **Mode (COOL / FAN / AIR_DRY) is not auto-managed**: the LG unit's native thermostat handles compressor cycling — when the room hits target in COOL mode, the unit shuts the compressor off and keeps the fan running. Earlier versions of the loop wrote COOL↔FAN every few minutes (~133 writes/day in production); that churn was removed. The loop only manages setpoint + fan speed + ladder priority. AIR_DRY mode (and any non-COOL/non-FAN mode) is detected as user intent and left alone. Manual mode changes (LG app, unit, or UI) still trigger the override detector and 30-min pause as designed.
