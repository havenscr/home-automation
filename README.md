...
- **COOL / FAN auto-switch**: when a room drops below its target, the loop switches that AC from COOL to FAN (compressor off, just airflow). When the room rises back above target, it switches back to COOL. A 0.5F deadband on each side prevents the compressor from cycling near setpoint. AIR_DRY mode is treated as user intent and left alone. After every mode switch, the loop force-rewrites the desired fan speed (LG portable ACs auto-reset fan on mode transitions) and the override detector skips fan-divergence checks for ~75s post-switch to avoid spurious pauses.
